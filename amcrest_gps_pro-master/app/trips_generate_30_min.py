...
class TripManualCalculation30MinFreq(APIView):
    ...
    def post(self, request):
        if request.data.get('imei'):
            ...
            frequency = request.data.get('frequency', 5)

            try:
                check_frequency_int = int(frequency) + 5
            except(Exception)as e:
                check_frequency_int = 5

            trip_calculation = CalculateGl24Trip30MinFreq(request.data.get('imei'), date, month, year, check_frequency_int)
...
class CalculateGl24Trip30MinFreq:

    def __init__(self, imei, day, month, year, frequency_int):
        self.imei_list = []
        self.trip_id = None
        ...
        self.month = month
        self.year = year
        self.frequency_int = frequency_int
                ...
                if details.get('gps_accuracy') not in ['0', 0] and details.get('mileage', None) and details.get('latitude', None) != 0 and details.get('longitude', None) != 0:
                    if not trip_log:
                        ...
                        if last_lat != details.get('latitude') and last_long != details.get('longitude'):
                            ...
                            if average_time_diff<self.frequency_int:
    ...
    def check_time_diff(self, imei, old_time, new_time):
        ...
        if time_diff>= self.frequency_int:#self.get_trip_timeout(imei):
            # print('trip timeout')
