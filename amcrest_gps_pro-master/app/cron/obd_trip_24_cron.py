...
from app.models import UserTrip, UserObdTrip, TripsMesurement, TripsObdMesurement, TripCalculationData, Subscription, SettingsModel, TripCalculationGLCron, TripCalculationObdCron
from app.serializers import UserTripSerializer, UserObdTripSerializer, TripsMesurementSerializer, TripsObdMesurementSerializer, TripCalculationDataSerializer, TripCalculationGLCronSerializer
...
class CalculateObd24Trip(CronJobBase):
    ...
    def do(self):
        ...
        for i in self.imei_list:
            ...
            if timezone:
                ...
                # try:
                #     if self.trip_id:
                #         _thread.start_new_thread(fuel_event_module, (self.trip_id,))
                # except(Exception)as e:
                #     print(e)
    ...
    def get_data(self, imei, time_date):
        ...
        try:
            ...
            ignition = IgnitionOnoff.objects.filter(imei=imei, send_time__gte=send_time).exclude(latitude = 0, longitude = 0).exclude(latitude = None, longitude = None).all()
        except(Exception)as e:
            ignition = IgnitionOnoff.objects.filter(imei=imei, record_date__gte=time_date).exclude(latitude = 0, longitude = 0).exclude(latitude = None, longitude = None).all()
        ...
        try:
            send_time = int(time_date.strftime("%Y%m%d%H%M%S"))
            obd = ObdMarkers.objects.filter(imei=imei, send_time__gte=send_time).exclude(latitude = 0, longitude = 0, speed=0).exclude(latitude = None, longitude = None).all()
        except(Exception)as e:
            obd = ObdMarkers.objects.filter(imei=imei, record_date__gte=time_date).exclude(latitude = 0, longitude = 0, speed=0).exclude(latitude = None, longitude = None).all()
    ...
    def calculate_trip(self, records, imei):
        ...
        user_trip = UserObdTrip.objects.filter(imei=imei, record_date_timezone__lte=datetime.datetime.strptime(to_date, '%Y-%m-%d %H:%M:%S'), record_date_timezone__gte=datetime.datetime.strptime(from_date, '%Y-%m-%d %H:%M:%S')).first()
        if user_trip:
            user_trip.delete()
        ...
        for record in records:
            if record.get('protocol', None):
                ...
                if ((details.get('protocol', None) == '+RESP:GTOBD' or details.get('protocol', None) == '+BUFF:GTOBD') and details.get('latitude', None) != 0 and details.get('longitude', None) != 0) or float(details.get('mileage'))!=last_mileage:
                    if not trip_log:
                    ...
                    if details.get('protocol', None) == '+RESP:GTIGN' or details.get('protocol', None) == '+BUFF:GTIGN':
                        if not trip_log:
                            last_send_time = str(details.get('send_time')) #20191121154837
                            last_mileage = float(details.get('mileage')) #1203.664
                            trip_log.append(details)
                        else:
                            trip_c = self.transfer_to_trip(imei, distance, time, trip_log)
                            trip_log = []
    ...
    def transfer_to_trip(self, imei, total_distance, total_time, log, start_time=None, end_time=None):
        ...
        if total_distance >= 0.1:
            ...
            # print(total_distance, total_time, log)
            if user_trip:
