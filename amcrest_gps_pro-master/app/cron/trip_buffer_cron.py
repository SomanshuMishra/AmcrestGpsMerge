...
class BufferTripCalculation(CronJobBase):
    RUN_EVERY_MINS = 0 # every 2 hours

    schedule = Schedule(run_every_mins=RUN_EVERY_MINS)
    code = 'app.trip_buffer_cron'    # a unique code
...
class BufferTripCalculationOBD(CronJobBase):
    RUN_EVERY_MINS = 0 # every 2 hours
    ...
    def get_imei(self):
        ...
        last_date = datetime.datetime.now() - datetime.timedelta(days = 1)
        last_date = last_date.date()
        fri = ObdMarkers.objects.filter(record_date__range=[last_date, today], gps_accuracy__gt=0, protocol="+BUFF:GTOBD").values('imei').distinct()
        return fri
                ...
                if (details.get('protocol', None) == '+RESP:GTOBD' or details.get('protocol', None) == '+BUFF:GTOBD') and float(details.get('mileage'))!=last_mileage and details.get('latitude', None) != 0 and details.get('longitude', None) != 0:
                    if not trip_log:
                        ...
                        if last_lat != details.get('latitude') and last_long != details.get('longitude'):
                            ...
                            # print(mean_time_diff, mean_time_diff/60)
    ...
    def transfer_to_trip(self, imei, total_distance, total_time, log, start_time=None, end_time=None):
        ...
        if total_distance > 0:
            ...
            if user_trip:
                ...
                customer_id = self.get_customer_id(imei)
                user_trip_obj = {'imei':imei, 'driver_id':'1', 'trip_log':[log], 'measure_id':10, 'record_date_timezone': self.user_start_time, 'start_time' : start_time, 'end_time' : end_time, 'customer_id':customer_id}
                serializer = UserTripSerializer(data = user_trip_obj)
                if serializer.is_valid():
                    serializer.save()
                    close_old_connections()
