...
def battery_notification_maker(args):
	...
	if battery_percentage != 0 and battery_percentage != None:
		report_instance = Reports.objects.filter(event_type='battery_lowest', imei=imei).order_by('-id').first()

		if battery_lowest_point > battery_percentage:
			try:
			...
			if report_battery_status or report_battery_status == None:
				...
				report_instance_delete = Reports.objects.filter(event_type='battery_lowest', imei=imei).all()
				...
				_thread.start_new_thread(send_notification, (imei, message_title, message_body, device_name.customer_id, report_obj, 'battery_lowest', global_send_notification_flag))
				...
				_thread.start_new_thread(save_notifications, (imei, message_title, message_body_to_save, device_name.customer_id, report_obj, 'battery_lowest', longitude, latitude))
